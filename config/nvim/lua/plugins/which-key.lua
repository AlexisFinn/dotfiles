local keys = {
  ...
  { "<leader>dd", "<cmd>bd<CR>", desc = "Delete current buffer", silent = true },
  {
    ...
    function()
      ...
      vim.api.nvim_exec_autocmds("User", { pattern = "DelOldBuf" })
    end,
  ...
  { "<leader>tp", function() vim.diagnostic.jump({ count = -1, float = true }) end, desc = "Go to Previous lsp error" },
  { "<leader>tn", function() vim.diagnostic.jump({ count = 1, float = true }) end, desc = "Go to Previous lsp error" },
